...
//go:embed shared_script_template.js
var SharedScriptTemplate string
...
const FormSharedJavascriptTemplate = `<script type="text/javascript">` +
	`{{.SharedJavascript}}` +
	`</script>`
...
type FormCoreEngine struct {
	...
	sharedJsTemplate TemplateIf
}

func NewFormCoreEngine(cssTemplate TemplateIf, htmlTemplate TemplateIf, jsTemplate TemplateIf, sharedJsTemplate TemplateIf) *FormCoreEngine {
	return &FormCoreEngine{
		cssTemplate:      cssTemplate,
		htmlTemplate:     htmlTemplate,
		jsTemplate:       jsTemplate,
		sharedJsTemplate: sharedJsTemplate,
	}
}
...
func (fce FormCoreEngine) GenerateSharedJavascript(data dtos.FormCoreData) (string, error) {
	var buf strings.Builder
	data.SharedJavascript = SharedScriptTemplate
	err := fce.sharedJsTemplate.Execute(&buf, data)
	if err != nil {
		return "", err
	}
	return buf.String(), nil
